package org.fms.auth.provider.config.auth.exception;
...
//        }
        if ( (e instanceof InvalidGrantException)&&e.getMessage().equals("Bad credentials")) {
            status=300;
            message=" 账号或密码不正确";
        }

        if ( (e instanceof UnauthorizedException)&&e.getMessage().contains("找不到该用户")) {
            status=300;
            message=" 账号或密码不正确";
        }

        MyOAuth2Exception exception = new MyOAuth2Exception(message);
